/**
 * Daily Principles — principles to live by.
 * Edit this list to update what shows on the left sidebar.
 */
export const PRINCIPLES: string[] = [
  'Treat others the way you want to be treated',
  'Stay true to your word,  don\'t make promises you cant keep',
  'Be just, practice kindness and gratitude, and walk humbly with God.',
  'Leave things better than you found them',
  'Work Hard, Play Hard',
  'Do no harm, it costs nothing to be nice, be honest, and be loyal',
  'Focus on the things you can control',
  'Anything worth doing is worth doing well',
  'Character over everything else',
  'Never stop learning and having new experiences',
  "You are a product of your habits and the people around you"
];
